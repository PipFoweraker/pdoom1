...
- **Decision log (the "what & why-not"):** [`docs/game-design/decisions/`](game-design/decisions/)  --  one ADR per decision.
  > WARNING The ADR index in [`decisions/README.md`](game-design/decisions/README.md) is
...
- **Contract:** every consumer passes its inline literal as the fallback, so a missing/broken
  file *should* degrade to shipped behavior. WARNING **This contract is currently violated for
  `doom.streams.*`**  --  see [section 7](#7-known-gaps--active-fronts).
...
| Change turn step order / add a sim step | `_step_*` in [`turn_manager.gd`](../godot/scripts/core/turn_manager.gd)  --  **WARNING order is replay-load-bearing** |
