...
class MockLogger:
    ...
    def step_success(self, name: str, **kwargs: Any) -> None:
        print(f"[SUCCESS] {name}")

    def step_failure(self, name: str, **kwargs: Any) -> None:
        print(f"[FAILURE] {name}")

    def file_operation(self, op: str, path: str, result: str, **kwargs: Any) -> None:
        print(f"[FILE] {op} {path}: {result}")

    def metrics(
        self, name: str, value: Union[str, int, float], unit: Optional[str] = None, **kwargs: Any
    ) -> None:
        print(f'[METRIC] {name}: {value} {unit or ""}')

    def session_summary(self, success: bool, **kwargs: Any) -> None:
        print(f"[SUMMARY] Success: {success}")


def get_logger(
    name: str, category: Optional[Any] = None, verbose: bool = False, structured: bool = False
) -> MockLogger:
    return MockLogger()
...
class IntelligentASCIIConverter:
    ...
    def __init__(self, verbose: bool = False, ignore_file: Optional[Path] = None):
        ...
        # Load ignore patterns from .asciiignore file
        if ignore_file is None:
            ignore_file = Path(__file__).parent.parent / ".asciiignore"
    ...
    def _load_ignore_patterns(self, ignore_file: Path) -> List[str]:
        ...
        if not ignore_file.exists():
            self.logger.warning(f"Ignore file not found: {ignore_file}")
            return patterns
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(patterns)} ignore patterns from {ignore_file.name}")
    ...
    def _should_ignore(self, file_path: Path, base_dir: Path) -> bool:
        ...
        try:
            ...
            # File is outside base_dir, don't ignore
            return False
        ...
        for pattern in self.ignore_patterns:
            ...
            # Handle exact file or directory match
            if fnmatch(rel_path_str, pattern):
                return True

            # Handle parent directory patterns
            for parent in rel_path.parents:
                parent_str = str(parent).replace("\\", "/")
                if fnmatch(parent_str, pattern.rstrip("/").rstrip("*")):
                    return True
    ...
    def convert_text(self, text: str) -> Tuple[str, List[str]]:
        ...
        if text != original_text:
            changes.append("Applied intelligent ASCII conversion")
            self.logger.metrics(
                "characters_converted",
                len(original_text) - len(text.encode("ascii", "ignore").decode("ascii")),
            )

        if remaining_unicode:
            changes.extend([f"Remaining Unicode: {char}" for char in remaining_unicode[:5]])
            self.logger.warning(
                f"Found {len(remaining_unicode)} remaining Unicode characters",
    ...
    def process_file(self, file_path: Path, dry_run: bool = False) -> Tuple[bool, List[str]]:
        ...
        self.logger.step_start(
            "process_file", extra_data={"file_path": str(file_path), "dry_run": dry_run}
        )
        ...
        if not dry_run:
            try:
                ...
                self.logger.file_operation(
                    "WRITE", str(file_path), "SUCCESS", extra_data={"changes_count": len(changes)}
                )
                self.logger.step_success("process_file", extra_data={"file_path": str(file_path)})
                return True, changes + ["File updated successfully"]
            except Exception as e:
                self.logger.error(
                    f"Error writing file: {e}", extra_data={"file_path": str(file_path)}
                )
                return False, [f"Error writing {file_path}: {e}"]

        self.logger.file_operation(
            "SIMULATE", str(file_path), "WOULD_UPDATE", extra_data={"changes_count": len(changes)}
        )
    ...
    def process_directory(self, directory: Path, dry_run: bool = False) -> Dict[str, List[str]]:
        ...
        for file_path in files_to_process:
            ...
            if self._should_ignore(file_path, directory):
...
Examples:
  ...
  python intelligent_ascii_converter.py --directory docs/
    ...
    parser.add_argument(
        "--structured", action="store_true", help="Use structured JSON output for CI/CD"
    )
    ...
    converter.logger.info(
        "ASCII Converter session started",
        extra_data={
    ...
    if args.file:
        ...
        with TimedOperation(converter.logger, "process_single_file"):
        ...
        with TimedOperation(
            converter.logger, "process_directory", extra_data={"directory": str(args.directory)}
        ):
        ...
        if not results:
            ...
            if args.dry_run:
                ...
                converter.logger.info(
                    "Conversion completed", extra_data={"files_updated": len(results)}
                )

    # Log session summary
    results_count = len(results) if "results" in locals() and results else 0
    converter.logger.session_summary(
        success,
        extra_data={
