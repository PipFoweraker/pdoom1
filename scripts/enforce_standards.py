...
class StandardsEnforcer:
    ...
    def check_documentation(self) -> bool:
        ...
        outdated_imports = self._find_files_with_pattern(
            "from game_state import", exclude_dirs=[".git", "scripts"]
        )
        if outdated_imports:
            ...
            try:
                ...
                if converter_path.exists():
                    ...
                    for file_path, line_num, char in unicode_files:
                        self.errors.append(
                            f"  {file_path}:{line_num} - Non-ASCII character: {repr(char)}"
                        )
                    success = False
    ...
    def check_import_patterns(self) -> bool:
        ...
        old_patterns = ["from game_state import", "from actions import", "from events import"]

        for pattern in old_patterns:
            matches = self._find_files_with_pattern(
                pattern, exclude_dirs=["docs", ".git", "scripts"]
            )
    ...
    def _find_files_with_pattern(
        self, pattern: str, exclude_dirs: List[str] = None, is_regex: bool = False
                    ...
                    for line_num, line in enumerate(f, 1):
                        if is_regex:
                            if pattern_re.search(line):
                                matches.append(
                                    (str(py_file.relative_to(self.project_root)), line_num, line)
                                )
                        else:
                            if pattern in line:
                                matches.append(
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            try:
                ...
                syntax_errors.append(
                    f"{py_file.relative_to(self.project_root)}:{e.lineno} - {e.msg}"
                )
    ...
    def _find_debug_code(self) -> List[str]:
        ...
        for pattern in debug_patterns:
            matches = self._find_files_with_pattern(
                pattern, exclude_dirs=["tests", "debug", "dev-blog"]
            )
