...
def run_command(cmd, cwd=None, capture_output=True):
    ...
    try:
        result = subprocess.run(
            cmd, shell=True, cwd=cwd, capture_output=capture_output, text=True, timeout=30
        )
...
def check_local_repo(repo_path):
    ...
    # Get last commit
    success, last_commit, _ = run_command("git log -1 --pretty=format:'%h %s (%cr)'", cwd=repo_path)
    if not success:
    ...
    if local_path:
        ...
        if local_success:
            ...
            if local_data["ahead"] > 0:
                print(
                    f"     {Colors.YELLOW}[UP_ARROW][EMOJI]  {local_data['ahead']} commits ahead{Colors.END}"
                )
            if local_data["behind"] > 0:
                print(
                    f"     {Colors.YELLOW}[DOWN_ARROW][EMOJI]  {local_data['behind']} commits behind{Colors.END}"
                )
        else:
...
def print_quick_actions():
    ...
    print(
        f"    {Colors.CYAN}git clone https://github.com/PipFoweraker/pdoom1-website.git{Colors.END}"
    )
    print()
...
def main():
    ...
    for repo in repositories:
        print_repo_status(repo["name"], repo["emoji"], repo["github"], repo.get("local"))
