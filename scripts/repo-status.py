...
def print_header():
    ...
    print(f"{Colors.BOLD}{Colors.CYAN}🎮 P(Doom) Ecosystem Status Dashboard{Colors.END}")
...
def print_repo_status(name, emoji, github_repo, local_path=None):
    ...
    if github_success:
        print(f"  {Colors.GREEN}✅ GitHub: Active{Colors.END}")
        ...
        print(f"  {Colors.RED}❌ GitHub: {github_data}{Colors.END}")
    ...
    if local_path:
        ...
        if local_success:
            print(f"  {Colors.GREEN}📁 Local: Available{Colors.END}")
            ...
            if local_data['ahead'] > 0:
                print(f"     {Colors.YELLOW}⬆️  {local_data['ahead']} commits ahead{Colors.END}")
            if local_data['behind'] > 0:
                print(f"     {Colors.YELLOW}⬇️  {local_data['behind']} commits behind{Colors.END}")
        else:
            print(f"  {Colors.RED}📁 Local: {local_data}{Colors.END}")
    
    print()

def check_sync_status():
    """Check GitHub Actions sync status"""
    print(f"{Colors.BOLD}🔄 Sync Status{Colors.END}")
    print("-" * 40)
    ...
    if success:
        try:
            ...
            print(f"  {Colors.RED}❌ Failed to parse workflow data{Colors.END}")
    else:
        print(f"  {Colors.RED}❌ Could not fetch workflow status{Colors.END}")
