...
def create_verbose_money_message(val: float, reason: str = "", current_money: float = 0) -> List[str]:
    ...
    if current_money < 100:
        messages.append("⚠️⚠️ [CASH FLOW ALERT] Reserves running critically low!")
...
def create_verbose_staff_message(val: float, reason: str = "", current_staff: int = 0) -> List[str]:
    ...
    if val > 0:  # Hiring staff
        if count >= 5:
            ...
            messages.append("⚡ The lab buzzes with fresh energy and ambitious conversations")
        elif count >= 2:
        ...
        if current_staff >= 20:
            ...
            messages.append("⚙️ [SOLID FOUNDATION] A capable core team forms the backbone of your operations")
...
def create_verbose_reputation_message(val: float, reason: str = "", current_rep: int = 0) -> List[str]:
    ...
    if val > 0:  # Gaining reputation
        if amount >= 10:
            messages.append(f"⭐ MAJOR RECOGNITION: +{amount} reputation from outstanding achievements!")
            messages.append("📰 Industry publications take notice of your groundbreaking work")
        elif amount >= 5:
            messages.append(f"⭐ SOLID PROGRESS: +{amount} reputation as your work gains recognition")
        else:
            messages.append(f"✨ Steady improvement: +{amount} reputation from consistent performance")
            
        # Add context based on reputation level
        if current_rep >= 80:
            messages.append("🌟 [INDUSTRY LEADER] Your reputation precedes you in all professional circles")
        elif current_rep >= 60:
            messages.append("🎖️ [WELL REGARDED] Your organization is widely respected in the field")
        elif current_rep >= 40:
            messages.append("📈 [RISING STAR] Your growing reputation opens new doors")
        ...
        if current_rep <= 20:
            messages.append("⚠️ [CREDIBILITY CRISIS] Your reputation makes future partnerships difficult")
        elif current_rep <= 40:
...
def create_verbose_compute_message(val: float, reason: str = "", current_compute: int = 0) -> List[str]:
    ...
    if val > 0:  # Gaining compute
        if amount >= 100:
            messages.append(f"🖥️ MASSIVE COMPUTE BOOST: +{amount} units of computational power!")
            messages.append("⚡ Your researchers celebrate the expanded capabilities")
        elif amount >= 50:
            messages.append(f"💻 SIGNIFICANT UPGRADE: +{amount} compute units enhance your capabilities")
        else:
            messages.append(f"⚙️ System improvement: +{amount} compute units added to your resources")
            
        # Add context based on compute level
        if current_compute >= 1000:
            messages.append("🚀 [SUPERCOMPUTING] Your computational resources rival major tech companies")
            ...
            messages.append("⚡ [SOLID FOUNDATION] Reliable computational resources support steady progress")
            
    else:  # Using/losing compute
        if amount >= 100:
            messages.append(f"🖥️💨 INTENSIVE COMPUTATION: -{amount} compute units consumed by major calculations")
        elif amount >= 50:
            messages.append(f"💻 HEAVY PROCESSING: -{amount} compute units used for complex analysis")
        else:
            messages.append(f"⚙️ Standard usage: -{amount} compute units consumed by routine operations")
...
def create_verbose_research_message(research_type: str, doom_reduction: int, rep_gain: int) -> List[str]:
    ...
    if research_type.lower() == "safety":
        messages.append(f"🛡️ SAFETY BREAKTHROUGH: Research reduces existential risk by {doom_reduction} points!")
        messages.append(f"📊 Reputation grows by {rep_gain} as the community recognizes your responsible approach")
        messages.append("🎯 Your commitment to safety research sets a positive example for the field")
    elif research_type.lower() == "capabilities":
        messages.append(f"🚀 CAPABILITIES ADVANCE: Significant progress in AI systems development!")
        messages.append(f"📈 Reputation increases by {rep_gain} from impressive technical achievements")
        if doom_reduction < 0:  # Capabilities research might increase doom
            messages.append(f"⚠️ However, rapid capability advances may increase long-term risks")
    else:
        messages.append(f"🔬 RESEARCH PROGRESS: {research_type} research yields valuable insights")
        messages.append(f"📊 Academic standing improves by {rep_gain} reputation points")
        if doom_reduction > 0:
            messages.append(f"🛡️ This work contributes to reducing existential risk by {doom_reduction}")
