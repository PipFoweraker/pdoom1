...
Examples:
  ...
  python tools/select_assets.py --file art_prompts/ui_icons.yaml --gallery generated
    ...
    parser.add_argument('--select', nargs='+', help='Quick select variants (format: asset_id:variant)')
    parser.add_argument('--list', dest='list_status', help='List assets with specified status')
    parser.add_argument('--gallery', dest='gallery_status', help='Open HTML gallery with assets of specified status')
    ...
    if args.gallery_status:
        ...
        print(f"   File: {gallery_path}")
        print(f"\n   Click variants to select, then copy commands at bottom.")
        print(f"   Apply with: python tools/select_assets.py --file {args.file} --select <copied commands>\n")
        return 0
