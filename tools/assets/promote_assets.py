    ...
    for size in sizes:
        ...
        if dry_run:
            if selected_variant:
                ...
                print(
                    f"  📋 Would copy: {source_file.name} → {dest_file.relative_to(dest_base_dir.parent)}"
                )
        else:
            ...
            if selected_variant:
                ...
                print(
                    f"  ✅ Copied: {source_file.name} → {dest_file.relative_to(dest_base_dir.parent)}"
                )
...
Workflow:
  ...
  3. Promote to game: python tools/promote_assets.py --file art_prompts/ui_icons.yaml
    ...
    parser.add_argument(
        "--status",
        default="selected",
        help="Only promote assets with this status (default: selected)",
    )
    ...
    parser.add_argument(
        "--dest",
        default="godot/assets/icons",
        help="Destination directory (default: godot/assets/icons)",
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be done without copying files"
    )
    parser.add_argument(
        "--mark-promoted", action="store_true", help="Update YAML to mark assets as promoted"
    )
    ...
    if not filtered:
        ...
        print("   Try using --status generated to see recently generated assets")
        return 0
    ...
    for i, asset in enumerate(filtered, 1):
        ...
        success, files = promote_asset(asset, source_dir, dest_dir, output_sizes, args.dry_run)
