...
class GitHubIssueSync:
    ...
    def find_matching_github_issue(
        self, local_issue: Dict, github_issues: List[Dict]
    ) -> Optional[Dict]:
    ...
    def sync_issues(self, dry_run: bool = True) -> Dict:
        ...
        print(
            f"[SEARCH] Analyzing {len(local_issues)} local vs {len(github_issues)} GitHub issues..."
        )
...
def main():
    ...
    parser.add_argument(
        "--create", action="store_true", help="Actually create missing GitHub issues"
    )
