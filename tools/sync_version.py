...
if any derived copy has drifted from ``version.txt`` so the two can never
silently disagree (the leaderboard board-key derives from the version, so a
silent drift would fork score boards).
"""
...
def _stamp_game_config(text: str, ver: str) -> str:
    return re.sub(
...
def _stamp_project_godot(text: str, ver: str) -> str:
...
def _stamp_export_presets(text: str, ver: str) -> str:
    quad = _four_part(ver)
...
def _stamp_welcome_scene(text: str, ver: str) -> str:
...
# (path, stamper) pairs. Each stamper is pure: (text, ver) -> new_text.
TARGETS = [
...
def main() -> int:
    ...
    if args.check:
        if out_of_sync:
            print(f"[sync_version] OUT OF SYNC with version.txt={ver}:")
            for p in out_of_sync:
                print(f"  - {rel(p)}")
            print("[sync_version] run: python tools/sync_version.py")
            return 1
        print(f"[sync_version] all targets in sync with version.txt={ver}")
        return 0

    if written:
        print(f"[sync_version] stamped version.txt={ver} into:")
        for p in written:
            print(f"  - {rel(p)}")
    else:
        print(f"[sync_version] already in sync with version.txt={ver}")
    return 0
