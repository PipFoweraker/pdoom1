def draw_intelligence_dialog(
    screen: pygame.Surface, intelligence_dialog: Dict[str, Any], w: int, h: int
) -> List[Dict[str, Any]]:
    """
    ...
    for option in intelligence_dialog["options"]:
        ...
        cost_rect = cost_surface.get_rect(
            x=button_rect.right - cost_surface.get_width() - 15, y=button_rect.y + 8
        )
        ...
        if option["available"]:
            clickable_rects.append(
                {"rect": button_rect, "type": "intelligence_option", "option_id": option["id"]}
            )
    ...
    clickable_rects.append({"rect": cancel_rect, "type": "cancel"})
