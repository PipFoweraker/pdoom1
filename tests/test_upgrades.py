...
class TestUpgradePurchasing(unittest.TestCase):
    ...
    def test_can_purchase_upgrade_with_sufficient_money(self):
        ...
        self.assertEqual(
            ...
            initial_money - upgrade["cost"],
            "Money should be deducted correctly",
        )
        self.assertIn(
            upgrade["effect_key"],
            self.game_state.upgrade_effects,
            "Upgrade effect should be added to effects set",
        )
        self.assertTrue(
            any("Upgrade purchased:" in msg for msg in self.game_state.messages),
    ...
    def test_cannot_purchase_upgrade_with_insufficient_money(self):
        ...
        self.assertFalse(
            upgrade.get("purchased", False), "Upgrade should not be marked as purchased"
        )
    ...
    def test_upgrade_success_messages(self):
        ...
        self.assertTrue(
            new_messages[0].startswith("Upgrade purchased:"),
            "Message should start with 'Upgrade purchased:'",
        )
        self.assertIn(upgrade["name"], new_messages[0], "Message should contain the upgrade name")
...
class TestUpgradeInitialization(unittest.TestCase):
    ...
    def test_upgrade_effects_initially_empty(self):
        ...
        self.assertEqual(
            len(game_state.upgrade_effects), 0, "Upgrade effects should be empty initially"
        )
        self.assertIsInstance(game_state.upgrade_effects, set, "Upgrade effects should be a set")
