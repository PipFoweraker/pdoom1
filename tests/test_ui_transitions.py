...
class TestUITransitions(unittest.TestCase):
    ...
    def test_multiple_transitions(self):
        ...
        transition1 = self.game_state._create_upgrade_transition(
            0, (100, 200, 150, 50), (650, 50, 45, 45)
        )
        transition2 = self.game_state._create_upgrade_transition(
            1, (100, 300, 150, 50), (600, 50, 45, 45)
        )
...
class TestUITransitionIntegration(unittest.TestCase):
    ...
    def test_transition_triggered_on_upgrade_purchase(self):
        ...
        if not upgrade.get("purchased", False) and self.game_state.money >= upgrade["cost"]:
            ...
            self.game_state.logger.log_upgrade(
                upgrade["name"], upgrade["cost"], self.game_state.turn
            )
    ...
    def test_extensible_transition_system(self):
        ...
        upgrade_transitions = [
            t for t in self.game_state.ui_transitions if t["type"] == "upgrade_transition"
        ]
        other_transitions = [
            t for t in self.game_state.ui_transitions if t["type"] != "upgrade_transition"
        ]
