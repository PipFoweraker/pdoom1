...
class TestActionPointsBackwardCompatibility(unittest.TestCase):
    ...
    def test_all_actions_have_ap_cost(self):
        ...
        for action in self.game_state.actions:
            ...
            if (
                "Set Research Quality" in action_name
                or "Research Speed:" in action_name
                or "Research Quality:" in action_name
            ):
    ...
    def test_action_costs_preserved(self):
        ...
        fundraise_action = next(
            action for action in self.game_state.actions if action["name"] == "Fundraising Options"
        )
...
class TestActionPointsStaffScaling(unittest.TestCase):
    ...
    def test_calculate_max_ap_complex_composition(self):
        ...
        # max AP = 3 + (10 * 0.5) + (3 * 1.0) = 11, but capped at 10
...
@unittest.skip(
    "TECH DEBT: Delegation system hardcoded for old action names, future release roadmap"
)
class TestActionPointsDelegation(unittest.TestCase):
    ...
    def test_delegation_fields_exist(self):
        ...
        safety_research = next(
            action for action in self.game_state.actions if action["name"] == "Safety Research"
        )
    ...
    def test_can_delegate_operational_action(self):
        ...
        buy_compute = next(
            action for action in self.game_state.actions if action["name"] == "Buy Compute"
        )
    ...
    def test_non_delegatable_action(self):
        ...
        fundraise = next(
            action for action in self.game_state.actions if action["name"] == "Fundraise"
        )
    ...
    def test_delegation_with_lower_ap_cost(self):
        ...
        buy_compute_idx = next(
            i for i, action in enumerate(self.game_state.actions) if action["name"] == "Buy Compute"
        )
    ...
    def test_auto_delegation_when_beneficial(self):
        ...
        if buy_compute_idx < len(action_rects):
            ...
            if (
                hasattr(self.game_state, "_action_delegations")
                and buy_compute_idx in self.game_state._action_delegations
            ):
                delegation_info = self.game_state._action_delegations[buy_compute_idx]
...
class TestEnhancedAPFeedback(unittest.TestCase):
    ...
    def test_error_tracking_for_easter_egg(self):
        ...
        error_messages = [
            msg for msg in self.game_state.messages if "Not enough Action Points" in msg
        ]
