...
def test_ui_fixes():
    ...
    try:
        ...
        print(
            f"   First-time help active: Spacebar {'WORKS' if spacebar_works else 'BLOCKED'} - {reason}"
        )
        ...
        spacebar_works, reason = test_spacebar(game_state, onboarding, None, "game")
        print(
            f"   Popup events pending: Spacebar {'WORKS' if spacebar_works else 'BLOCKED'} - {reason}"
        )
        ...
        assert debug_info["pending_popup_events"] is True
