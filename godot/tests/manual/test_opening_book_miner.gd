...
## Attention pass pending + a stream re-calibration owed. The MINER is the durable deliverable
## -- re-mine whenever constants move (one command). Beam/hill-climb refinement over promising
## prefixes: next step, worthwhile now that the fitness surface has a gradient (runs last months).
...
func test_opening_book_miner() -> void:
	...
	lines.append("")
	lines.append("> **Base:** dials-1-4 calibration on **nine-stream doom** (ADR-0015 / #643) -- runs last months,")
	lines.append("> so the %d-month random prefix is a real opening line, not a single doomed month. **CAVEAT:**" % OPENING_MONTHS)
	lines.append("> dial-5 Attention pass pending + a stream re-calibration owed; the MINER is the durable")
	lines.append("> deliverable -- re-mine whenever constants move (one command, footer).")
	lines.append("")
